package ru.arvalon.rx.chapter6;
...
public class FileBrowserViewModel {
    ...
    public void subscribe() {
        ...
        final BehaviorSubject<File> selectedFile = BehaviorSubject.createDefault(fileSystemRoot);

        Observable<File> previousFileObservable = previousClickObservable.map(event -> selectedFile.getValue().getParentFile());

        Observable<File> rootFileObservable = rootClickObservable.map(event -> fileSystemRoot);
        ...
        subscriptions.add(selectedFile
                .switchMap(file -> getFiles.apply(file).subscribeOn(Schedulers.io()))
