package ru.arvalon.rx.chapter3;
...
import ru.arvalon.rx.chapter3.utils.CardType;
import ru.arvalon.rx.chapter3.utils.ValidationUtils;
...
public class Chapter3Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Observable<String> creditCardNumber = RxTextView.textChanges(creditCardNumberView).map(CharSequence::toString);

        final Observable<String> creditCardCvc = RxTextView.textChanges(creditCardCvcView).map(CharSequence::toString);

        final Observable<String> creditCardExpirationDate = RxTextView.textChanges(creditCardExpirationDateView).map(CharSequence::toString);

        // Create derived Observables
        final Observable<CardType> cardType = creditCardNumber.map(CardType::fromNumber);

        final Observable<Boolean> isKnownCardType = cardType.map(cardTypeValue -> cardTypeValue != CardType.UNKNOWN);

        final Observable<Boolean> isValidCheckSum = creditCardNumber.map(ValidationUtils::checkCardChecksum);
        ...
        final Observable<Boolean> isValidExpirationDate = creditCardExpirationDate.map(text -> expirationDatePattern.matcher(text).find());
    ...
    private static void showErrorForEditText(EditText editText, Observable<Boolean> isValid) {
    ...
    private static Observable<Boolean> getShowErrorForEditText(EditText editText, Observable<Boolean> isValid) {
    ...
    private void resolveViews() {
        ...
        creditCardNumberView = findViewById(R.id.credit_card_number);
        creditCardCvcView = findViewById(R.id.credit_card_cvc);
        creditCardType = findViewById(R.id.credit_card_type);
        creditCardExpirationDateView = findViewById(R.id.expiration_date);
